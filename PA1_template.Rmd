...
The median is `r format(medianSteps,nsmall=2)`. The mean is `r format(meanSteps, nsmall=2)`. The previous histogram gives the intuition that days without measurements bring those averages down by providing 0 values in the calculation.
...
* For the mean, we get `r format(meanFilledSteps, nsmall=2)` instead of `r format(meanSteps, nsmall=2)`
* For the median, we get `r format(medianFilledSteps, nsmall=0)` instead of  `r format(medianSteps, nsmall=0)`
